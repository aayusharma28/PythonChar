...
def plotfive(dropdown1_value):
    app.layout = html.Div(id = 'parent', children = [
    html.Div([
    html.H1(id = 'H1', children = 'YTM vs TIME (Multiple)', style = {'textAlign':'center',\
                                            'marginTop':40,'marginBottom':40}),
    ...
    for i in range(0, len(dropdown1_value)):
        df.append(dataframe(dropdown1_value[i]))
    if len(df) == 5:
        dfs = {"df1": df[0], "df2": df[1], "df3": df[2], "df4": df[3], "df5": df[4]}
        fig = go.Figure()
        for i in dfs:
        ...
        dfs = {"df1": df[0], "df2": df[1], "df3": df[2], "df4": df[3]}
        fig = go.Figure()
        for i in dfs:
            fig = fig.add_trace(go.Scatter(x = dfs[i]["Month"],
                                       y = dfs[i]["YTM"], 
        ...
        dfs = {"df1": df[0], "df2": df[1], "df3": df[2]}
        fig = go.Figure()
        for i in dfs:
            fig = fig.add_trace(go.Scatter(x = dfs[i]["Month"],
                                       y = dfs[i]["YTM"], 
